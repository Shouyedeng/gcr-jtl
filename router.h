...
extern int rtrPinArrayLink(GCRPin *pins, int nPins);
extern int rtrPinArrayFixStems(GCRPin *pins, int nPins);
extern int rtrPinArrayInit(GCRChannel *ch, int side, GCRPin *pins,int nPins, int net_id[]);
extern void gcrCheckCol(GCRChannel *ch, int c, char *where);
extern int gcrTryRun(GCRChannel *ch, GCRNet *net, int from, int to, int column);
extern void gcrMoveTrack(GCRColEl *column, GCRNet *net, int from, int to);
extern int gcrRealDist(GCRColEl *col, int i, int dist);
extern int gcrClass(GCRNet *net, int track);
extern void gcrShellSort(GCRNet **v, int n, bool isUp);
extern void gcrUncollapse(GCRChannel *ch, GCRColEl **col, int width, int bot, int top, int split);
...
extern void gcrVacate(GCRChannel *ch, int column);
extern void gcrReduceRange(GCRColEl *col, int width);
extern void gcrPickBest(GCRChannel *ch);
extern void gcrCollapse(GCRColEl **col,int width, int bot, int top, int freed);

extern void gcrWanted(GCRChannel *ch, int track, int column);
extern void gcrMakeFeasible(GCRColEl *col, GCRNet *net, int from, int target, int width);
extern bool gcrBlocked(GCRColEl *col, int i, GCRNet *net, int last);
extern void gcrMarkWanted(GCRChannel *ch);
extern void gcrFeasible(GCRChannel * ch, int col);
extern void DumpNetPath(GCRChannel *ch, FILE *fp);
extern void freeChannel(GCRChannel *ch);
extern void gcrPrintCol(GCRChannel *, int, int);
extern bool get_net_id(char *s, int *pid);

extern void gcrInitCol(GCRChannel *ch, GCRPin *edgeArray);
extern void gcrSetFlags(GCRChannel *ch);
extern void gcrInitCollapse(int size);
extern int gcrDensity(GCRChannel *ch);
extern void gcrSetEndDist(GCRChannel *ch);
extern void gcrBuildNets(GCRChannel	* ch, FILE *outfp);
extern void gcrUnlinkPin(GCRPin *pin);
extern void gcrEvalPat(GCRColEl **col, int freed, int size);
extern void gcrLinkTrack(GCRColEl *col, GCRNet *net, int track, int width);
extern bool gcrVertClear(GCRColEl *col, int from, int to);
extern int gcrNextSplit(GCRColEl *col, int size, int i);
