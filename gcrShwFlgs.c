...
/* Figure out which channel is selected */
    ...
    {
	printf("Sorry.  You must route before looking at flags!\n");
	return;
    }
    tile = TiSrPoint((Tile *) NULL, RtrChannelPlane, point);
    if(TiGetType(tile) != 0)
    {
	printf("Point to the channel you want to highlight.\n");
	return;
    }
    he = HashLookOnly(&RtrTileToChannel, (char *) tile);
    if(he == (HashEntry *) NULL)
    {
	printf("No channel under point.  Have you already routed?\n");
	return;
    }
...
/* Translate the command argument to the bit mask for the flag */
	...
	if (mask == -1)
	    printf("%s:  ambiguous.  Legal values are:\n", arg);
	else
	    printf("%s:  not found.  Legal values are:\n", arg);
    ...
    {
	printf("Oops.  Somebody deleted the results array.\n");
	return;
    }
 ...
 */
	...
	{
	    printf("Oops.  Result array column %d is missing.\n", col);
	    return;
	}
    ...
    GCRChannel * ch;
{
    ...
    {
	printf("Can't open file %s to dump channel.\n", name);
	return;
    }
