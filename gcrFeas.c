    ...
    int 	 col;		/* Column for feasible connections	*/
{
	...
	if(topNet != (GCRNet *) NULL)
		if(topNet->routeflag == 0)
			topNet = (GCRNet *) NULL;
    botNet=ch->gcr_bPins[col].gcr_pId;
	if(botNet != (GCRNet *) NULL)
		if(botNet->routeflag == 0)
			botNet = (GCRNet *) NULL;
    column=ch->gcr_lCol;
 ...
 */
			...
			!(failedTop) && (botNet != (GCRNet *)NULL) && (botTarget >= topTarget))
		{
			...
			else if (botTarget > (ch->gcr_width - topTarget - 1))
			{
				gcrMakeFeasible(column, topNet, ch->gcr_width + 1, topTarget,
								ch->gcr_width);
				failedBot = TRUE;
			...
			}//end if (!(failedBot) && (topNet != (GCRNet *)NULL) && !(failedTop) && (botNet != (GCRNet *)NULL) && (botTarget >= topTarget))
		}
		else
