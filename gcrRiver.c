    ...
    GCRChannel *ch;
{
	...
	{
	    printf("Failing because top or bottom pins are used\n");
	    return (FALSE);
	}
	...
	if (USED(&ch->gcr_lPins[row]) && USED(&ch->gcr_rPins[row]))
	{
	    ...
	    {
		printf("Failing because left and right pins don't match\n");
		return (FALSE);
	    }
	}
    ...
    GCRChannel *ch;
{
	...
	if (USED(&ch->gcr_lPins[row]) || USED(&ch->gcr_rPins[row]))
	{
	    printf("Failing because left or right pins are used\n");
	    return (FALSE);
	}
	...
	if (USED(&ch->gcr_tPins[col]) && USED(&ch->gcr_bPins[col]))
	{
	    ...
	    {
		printf("Failing because top and bottom pins don't match\n");
		return (FALSE);
	    }
	}
