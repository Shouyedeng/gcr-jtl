...
void
RtrPinsInit(ch, net_id_l, net_id_r, net_id_t, net_id_b)
    GCRChannel *ch;
    int net_id_l[];
    int net_id_r[];
    int net_id_t[];
    int net_id_b[];
{
    rtrPinArrayInit(ch, GEO_NORTH, ch->gcr_tPins, ch->gcr_length, net_id_t);
    rtrPinArrayInit(ch, GEO_SOUTH, ch->gcr_bPins, ch->gcr_length, net_id_b);
    rtrPinArrayInit(ch, GEO_WEST, ch->gcr_lPins, ch->gcr_width, net_id_l);
    rtrPinArrayInit(ch, GEO_EAST, ch->gcr_rPins, ch->gcr_width, net_id_r);
}

int
rtrPinArrayInit(ch, side, pins, nPins, net_id)
    GCRChannel *ch;
	int side;
    GCRPin *pins;
    int nPins;
    int net_id[];
{
	...
	switch (side)
	{
        ...
        printf("pin->gcr_x=%d y=%d, pSeg=%d\n", pin->gcr_x, pin->gcr_y, pin->gcr_pSeg);
	pin->gcr_point = point;
