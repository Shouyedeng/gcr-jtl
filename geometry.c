			 ...
			 */
{
	...
	switch (pos)
	{
		...
		break;
	    case -2:
		printf("\"%s\" is not a valid direction or position.\n",
		    name);
		break;
	}
    }
    ...
    {
	printf("\"%s\" is not a Manhattan direction or position.\n", name);
	pos = -2;
    }
    printf("Legal directions/positions are:\n\t");
    for (fmt = "%s", pp = positions; pp->pos_name; pp++)
    {
	if (manhattan && !pp->pos_manhattan)
	    continue;
	printf(fmt, pp->pos_name);
	fmt = ",%s";
    }
    printf("\n");
    return (pos);
